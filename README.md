# CSE 326: Microprocessor and Microcontroller

This repository contains my notes, lab work, Assembly programs, and microcontroller experiments for **Microprocessor and Microcontroller**.


## Course Info
```
Course Title  : Microprocessor and Microcontroller
Course Code   : CSE 325 (Theory), CSE 326 (Lab)
Course Credit : 3 (Theory), 1.5 (Lab)
Session       : Summer 2026
```

## Course Instructor

- **Teacher:** Nourin Khandaker (NKD)
- **Designation:** Lecturer, Dept of CSE
- **Email:** nourin_khandaker@bubt.edu.bd

## Index

| Class | Theory Date | Theory Topic | Lab Date | Lab Topic |
|---:|---|---|---|---|
| 01 | 11 May 2026 | Introduction | 12 May 2026 | [Single and Double Character Input/Output](lab/lab-01-single-character-io) |
| 02 | 14 May 2026 | Microprocessor Architecture, Registers, and Memory Addressing | 19 May 2026 | [String Input/Output](lab/lab-02-string-input-output) |
| 03 | 18 May 2026 | Real Mode: Physical Address, Logical Address, and Address Conversion | 09 June 2026 | [If-else problems](lab/lab-03-if-else-problems) |
| 04 | 21 May 2026 | Protected Mode: Memory Addressing, Descriptors, and Descriptor Access | 16 June 2026 | [For loop problems](lab/lab-04-for-loop-problems) |
| 05 | 04 June 2026 | Chapter 03: Addressing Mode | 06 July 2026 | [Arduino Basics](lab/lab-05-arduino-basics) |
| 06 | 08 June 2026 | Chapter 04: Data Movement Instructions | — | — |
| 07 | 11 June 2026 | Chapter 02: Introduction to Microcontroller | — | — |
| 08 | 18 June 2026 | Chapter 02: 8051 Stack | — | — |
| 09 | 22 June 2026 | Chapter 03: Delay Calculation | — | — |
| 10 | 09 July 2026 | **MicroController** Chapter 08: Hex File | — | — |

## How to Install EMU8086

Follow these simple steps to install EMU8086 on Windows.

### Step 01: Download the Installer

Click the link below to download the EMU8086 installer:

[Download EMU8086 Installer](https://emu8086-microprocessor-emulator.en.softonic.com/download)

### Step 02: Run the Installer

After downloading, open the downloaded `.exe` installer file.

If Windows shows a security warning, continue only if you are sure the file is from a trusted source.

### Step 03: Add License Information

When EMU8086 asks for registration, enter your valid license information.

> ⚠️ Use only a valid license key provided by your teacher, university, or official purchase.

## Resources

### Reference Books

1. **The Intel Microprocessors: Architecture, Programming and Interfacing**  
   Barry B. Brey

2. **The 8051 Microcontroller and Embedded Systems**  
   Muhammad Ali Mazidi and Janice Gillispie Mazidi

3. **Assembly Language Programming and Organization of the IBM PC**  
   Ytha Yu and Charles Marut